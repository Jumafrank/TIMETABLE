...
//checking for empty slots in semester's timetable
for ($i = 3; $i < 9; $i += 2) {
    for ($k = 0; $k < 6; $k++) {
        for ($j = 0; $j < 5; $j++) {
...
//check for hunging in teacher's timetable
for ($i = 0; $i < $count; $i++) {
    for ($k = 0; $k < 6; $k++) {
        for ($j = 0; $j < 5; $j++) {
...
//Fetching detais of practical courses
...
//Storing semesters timetable into database
...
//Storing teachers timetable into database
for ($i = 0; $i < $teachers_count; $i++) {
...
//redirecting to generate timetable
